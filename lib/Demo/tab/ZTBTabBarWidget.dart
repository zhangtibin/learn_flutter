...
class ZTBTabBarWidget extends StatefulWidget {
  static const int BOTTOM_TAB = 1; //底部tab
  static const int TOP_TAB = 2; //顶部tab
  /*
  static
  类似java中的 static，表示一个成员属于类而不是对象
      final
  ...
  */
  ...
  @override
  _ZTBTabBarWidgetState createState() => _ZTBTabBarWidgetState(
      type,
      tabItems,
      tabViews,
      backgroundColor,
...
class _ZTBTabBarWidgetState extends State<ZTBTabBarWidget>
  ...
  Widget build(BuildContext context) {
    //顶部tabBar的模式
    if (this._type == ZTBTabBarWidget.TOP_TAB) {
    ...
    } else if (this._type == ZTBTabBarWidget.BOTTOM_TAB) {
      //如果选择的是底部tab
      return new Scaffold(
        //设置侧边滑出 drawer，不需要可以不设置
        drawer: _drawer,
